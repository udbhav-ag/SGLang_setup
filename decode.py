...
def load_model(server_args, port_args, gpu_id, tp_rank):
    ...
    suppress_other_loggers()
    print(f"Loading model on TP rank {tp_rank} with GPU ID {gpu_id}...")
...
def load_persisted_kv_for_reqs(
    ...
    """
    kv_load_cuda_only = os.environ.get("SGLANG_KV_LOAD_CUDA_ONLY", "0") == "1"
    restored_tokens, timing = load_batch_kv_cache_from_disk(
        reqs,
        model_runner,
        kv_load_dir,
        return_timing=True,
        cuda_only=kv_load_cuda_only,
    )
    rank_print(
        f"Loaded KV cache for {len(restored_tokens)} request(s) from {kv_load_dir}"
    )
    rank_print(f"KV load mode: {'cuda_only' if kv_load_cuda_only else 'cpu_staging'}")
    rank_print(
        "KV load timing: "
        f"disk={timing['disk_s']:.4f}s, "
        f"to_memory={timing['to_memory_s']:.4f}s, "
        f"total={timing['total_s']:.4f}s, "
        f"tokens={timing['num_tokens']}"
    )
...
def correctness_test(
    ...
    try:
        ...
        for i, req in enumerate(reqs):
            req.origin_input_ids = list(input_ids[i])
        ...
        # Start generation from loaded cache:
        # use all loaded prefix except the last prompt token, then run one extend.
        for req in reqs:
            if len(req.prefix_indices) > len(req.fill_ids):
                req.prefix_indices = req.prefix_indices[: len(req.fill_ids)]
        ...
        output_ids = [input_ids[i] + [next_token_ids_list[i]] for i in range(len(input_ids))]
